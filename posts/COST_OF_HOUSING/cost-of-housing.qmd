...
(1) and (2) are the most important metrics and incorporate (3) and (4). However, (3) and (4) can help provide some context to better understand (1) and (2).
...
What complicates this analysis is that the *typical* house from 1950 looks much different than the *typical* house from today. Across the decades, houses look pretty different, whether that be square footage or amenities. Nano Banana^[[Nano Banana](https://gemini.google/overview/image-generation/) is an AI image generator from Google. I thought these images were pretty good, but if you want some real examples you could peruse the book [House and Plans, 1950](https://archive.org/details/YorkHouseandPlans19500001) and just look at new builds on [Zillow](https://www.zillow.com/homes/).] can give us a visual sense of these differences: 

::: {#fig-housing-comparison layout-ncol=2}
![](/posts/COST_OF_HOUSING/images/1950s-home.png)

![](/posts/COST_OF_HOUSING/images/2020s-home.png)

Visual comparison of typical homes from the 1950's (left) and 2020's (right)
:::
...
[Andrew Latham](https://www.supermoney.com/author/andrew-latham) did a [great analysis on housing affordability](https://www.supermoney.com/inflation-adjusted-home-prices) where he found that the price per square foot of a house (ignoring amenities) from 1978-2023 grew by 20% which works out to roughly 0.44 pp per year which is about `r round(0.44/((coef(hp_data$mod)[2]*365/hp_data$tb$home_real[1])*100), 2)` the rate of house prices in general. 

Of course the problem in all this, is you can't buy a 1950's house today^[Not that anyone would want to!]; the option for a smaller house just isn't available. Those aren't the houses being built^[The median square footage of new builds has been [decreasing over the last decade](https://fred.stlouisfed.org/series/COMPSFLAM1FQ). Hopefully this is a trend that will continue if there is an appetite for it.], so buyers are left with nicer, more expensive houses.
